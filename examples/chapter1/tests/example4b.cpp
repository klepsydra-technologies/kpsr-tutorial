...
TEST(SumVectorDataTest, NominalPoolTest) {
    kpsr::EventEmitterMiddlewareProvider<std::vector<float>> vectorProvider(nullptr, "vector_provider", 10, [](std::vector<float> & element) {element.resize(10);}, nullptr);
    
    kpsr::EventEmitterMiddlewareProvider<float> sumProvider(nullptr, "sum_provider", 10, [](float & element){element = 0.0f;}, nullptr);
