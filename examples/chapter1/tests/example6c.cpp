...
TEST(ApplicationServiceTest, nominalTest) {
    ...
    kpsr::EventEmitterMiddlewareProvider<float> batteryValueProvider(nullptr, "battery_provider", 10, [](float & element) {element= 0.0f;}, nullptr);
    kpsr::EventEmitterMiddlewareProvider<kpsr::SystemEventData> statusProvider(nullptr, "status_provider", 10, nullptr, nullptr);

    kpsr::ControlService controlService(batteryValueProvider.getSubscriber(), statusProvider.getPublisher(), &environment);
    controlService.startup();
    kpsr::ApplicationService applicationService(&environment, statusProvider.getSubscriber(), "application");
    kpsr::mem::CacheListener<kpsr::SystemEventData> eventListener;
    statusProvider.getSubscriber()->registerListener("cacheListener", eventListener.cacheListenerFunction);
