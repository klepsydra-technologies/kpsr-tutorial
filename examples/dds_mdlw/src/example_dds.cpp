...
int main()
{
    ...
    kpsr::Publisher<kpsr::SystemEventData> *statusPublisher =
        toDDSProvider.getToMiddlewareChannel<kpsr::SystemEventData,
                                             kpsr_dds_serialization::LongData>(statusTopicName,
                                                                               0,
                                                                               nullptr,
                                                                               &statusDataWriter);

    // Creation of the From DDS Provider. It is a one-liner only needed once for
    // all readers.
    ...
    fromDDSProvider.registerToTopic(batteryTopicName,
                                    &batteryDataReader,
                                    true,
                                    provider.getPublisher<kpsr::sensors::BatteryState>(
                                        batteryTopicName, POOL_SIZE, nullptr, nullptr));

    // Finally, we pass the associated subscriber to the service that is going to
    // use. Please note that this is Transparent of the DDS completely, and yet
    // extremely performing.
    kpsr::ControlService controlService(&environment,
                                        provider.getSubscriber<kpsr::sensors::BatteryState>(
                                            batteryTopicName),
                                        provider.getSubscriber<kpsr::sensors::Temperature>(
                                            temperatureTopicName),
                                        statusPublisher);

    // Creating a concrete DDS reader and connect it to the publisher side
    dds::sub::DataReader<kpsr_dds_serialization::LongData> statusDataReader(sub, topicStatus);
    fromDDSProvider.registerToTopic(statusTopicName,
                                    &statusDataReader,
                                    true,
                                    provider.getPublisher<kpsr::SystemEventData>(statusTopicName,
                                                                                 POOL_SIZE,
                                                                                 nullptr,
                                                                                 nullptr));
    ...
    std::thread batterySensorThread([&pub, &toDDSProvider, &topicBattery, &batteryTopicName] {
        ...
        kpsr::Publisher<kpsr::sensors::BatteryState> *batteryPublisher =
            toDDSProvider.getToMiddlewareChannel<kpsr::sensors::BatteryState,
                                                 kpsr_dds_sensors::BatteryState>(batteryTopicName,
                                                                                 POOL_SIZE,
                                                                                 nullptr,
                                                                                 &batteryDataWriter);
    ...
    std::thread temperatureSensorThread([&pub,
                                         &toDDSProvider,
                                         &topicTemperature,
                                         &temperatureTopicName] {
        dds::pub::DataWriter<kpsr_dds_sensors::Temperature> temperatureDataWriter(pub,
                                                                                  topicTemperature);
        kpsr::Publisher<kpsr::sensors::Temperature> *temperaturePublisher =
            toDDSProvider
                .getToMiddlewareChannel<kpsr::sensors::Temperature, kpsr_dds_sensors::Temperature>(
