...
kpsr::ControlServiceRos::ControlServiceRos(Environment * environment,
...
void kpsr::ControlServiceRos::start() {
    std::function<void(sensor_msgs::BatteryState)> batteryListenerFn = std::bind(
        &ControlServiceRos::onBatteryReceived, this, std::placeholders::_1);
    _batterySubscriber->registerListener("batteryCacheListener",
                                             batteryListenerFn);

    std::function<void(sensor_msgs::Temperature)> temperatureListenerFn = std::bind(
        &ControlServiceRos::onTemperatureReceived, this, std::placeholders::_1);
    _temperatureSubscriber->registerListener("temperatureCacheListener",
                                                 temperatureListenerFn);
}
