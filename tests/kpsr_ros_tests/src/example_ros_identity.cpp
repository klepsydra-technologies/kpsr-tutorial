...
int main() {
    ...
    fromRosProvider.registerToTopic<sensor_msgs::BatteryState>(
        ...
        ROS_QUEUE_SIZE,
        provider.getPublisher<sensor_msgs::BatteryState>(batteryTopicName, POOL_SIZE, nullptr, nullptr));

    kpsr::ControlServiceRos controlService(&environment,
                                           provider.getSubscriber<sensor_msgs::BatteryState>(batteryTopicName),
                                           provider.getSubscriber<sensor_msgs::Temperature>(temperatureTopicName),
                                           kpsrStatusPublisher);
    ...
    std::thread batterySensorThread(
        [ &rate, &batteryTopicName, &nodeHandle]{
            ros::Publisher batteryPublisher = nodeHandle.advertise<::sensor_msgs::BatteryState>(batteryTopicName, 1);
    ...
    std::thread temperatureSensorThread(
        [ &rate, &temperatureTopicName, &nodeHandle]{
            ros::Publisher temperaturePublisher = nodeHandle.advertise<sensor_msgs::Temperature>(temperatureTopicName, 1);
