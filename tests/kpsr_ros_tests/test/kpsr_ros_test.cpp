...
TEST(KprsRosTest, batteryTest) {
    ...
    kpsr::EventEmitterMiddlewareProvider<kpsr::sensors::BatteryState> batteryProvider(
        nullptr, batteryTopicName, POOL_SIZE, nullptr, nullptr);
    kpsr::EventEmitterMiddlewareProvider<kpsr::sensors::Temperature> temperatureProvider(
        nullptr, temperatureTopicName, POOL_SIZE, nullptr, nullptr);
    kpsr::EventEmitterMiddlewareProvider<kpsr::SystemEventData> statusProvider(
        nullptr, statusTopicName, POOL_SIZE, nullptr, nullptr);
    ...
    kpsr::Publisher<kpsr::SystemEventData> * kpsrStatusPublisher = toRosProvider.getToMiddlewareChannel<
        kpsr::SystemEventData, std_msgs::Int32>(statusTopicName.c_str(), 1, nullptr, statusPublisher);
    ...
    std::thread batterySensorThread(
        [ &rate, &batteryTopicName, &nodeHandle, &sampleBatteryValue]{
            ros::Publisher batteryPublisher = nodeHandle.advertise<sensor_msgs::BatteryState>(batteryTopicName.c_str(), 1);
